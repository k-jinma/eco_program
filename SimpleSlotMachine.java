...
public class SimpleSlotMachine {
    public static void main(String[] args) {
        ...
        // 回転中アニメーション（擬似的）
        // このループ回数は見た目の調整用です
        for (int i = 0; i < 5; i++) { // 外側のループでアニメーションの繰り返し
            for (int j = 0; j < 3; j++) { // 内側のループで表示文字の変更
                char symbol;
                if (j == 0) symbol = '/';
                else if (j == 1) symbol = '-';
                else symbol = '\\';
                System.out.print("\b" + symbol); // バックスペースで1文字消して上書き

                // 擬似的な遅延 (非常に単純なもの)
                // Thread.sleep()が未学習のため、単純なループで代用
                for (long k = 0; k < 1200000000; k++) { // この回数は環境によって調整
                    // 何もしないで時間を消費
                }
            }
        }
        System.out.println("\b "); // アニメーションの最後の文字を消す
